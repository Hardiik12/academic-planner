...
export const PlannerProvider = ({ children }) => {
  ...
  const addToSemester = (semester, course) => {
    setPlannedCourses((prev) => {
      ...
      if (currentCredits + course.credits > MAX_CREDITS_PER_SEM) {
        alert(`Max ${MAX_CREDITS_PER_SEM} credits allowed per semester.`);
        return prev;
      }

      // Default grade set to 4 (A / 4.0 GPA scale default)
      return {
        ...prev,
        [semester]: [...existing, { ...course, grade: course.grade ?? 4 }],
      };
    });
  };
  ...
  const updateCourseGrade = (semester, id, grade) => {
    setPlannedCourses((prev) => {
      const list = prev[semester] || [];
      const updated = list.map((c) =>
        c.id === id ? { ...c, grade: Number(grade) } : c
      );
      return {
        ...prev,
        [semester]: updated,
  ...
  const getCourseById = (id) => {
    return initialCourses.find((c) => c.id === id);
  };
